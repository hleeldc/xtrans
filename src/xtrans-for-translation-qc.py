...
class Xtrans(QMainWindow):
    def __init__(self, reverse=False):
        ...
        hbox = QHBox(splitter)  # upper panel
        hboxLeft = QHBox(hbox)
        self.tred = TranscriptEdit(hbox)
        f = self.tred.font()
        f.setPointSize(16)
        self.tred.setFont(f)
        if reverse:
            self.tred.setVScrollBarMode(QScrollView.AlwaysOff)
            self.sb2 = self.tred.verticalScrollBar2(hboxLeft)
            self.subx = SuSideBar(self.tred, parent=hbox, xtrans=self)
            self.spkrbx = SpeakerSideBar(
                self.tred, 'speaker', '%s', self.colorMap.color, hbox, xtrans=self)
            self.secbx = SectionSideBar(self.tred, parent=hbox, xtrans=self)
        else:
            self.secbx = SectionSideBar(self.tred, parent=hboxLeft, xtrans=self)
            self.spkrbx = SpeakerSideBar(
                self.tred, 'speaker', '%s', self.colorMap.color, hboxLeft, xtrans=self)
            self.subx = SuSideBar(self.tred, parent=hboxLeft, xtrans=self)
        ...
        hbox.setStretchFactor(self.tred, 1)
        ...
        # speaker panel
        self.spkrPanel = QVBox(hbox)
        self.spkrSelPanel = SpeakerSelectionPanel(
            self.colorMap.color, self.spkrPanel)
        spkrButtons = QButtonGroup(4,Qt.Horizontal,self.spkrPanel)
        spkrButtons.setInsideSpacing(3)
        spkrButtons.setInsideMargin(3)
        btnVOS = SmallButton("VOS", spkrButtons)
        btnVAS = SmallButton("VAS", spkrButtons)
        btnSRT = SmallButton("SRT", spkrButtons)
        btnCLR = SmallButton("CLR", spkrButtons)
        btnNSI = SmallButton("NSI", spkrButtons)
        btnESIg = SmallButton("ESIg", spkrButtons)
        btnMRG = SmallButton("MRG", spkrButtons)
        btnMRGg = SmallButton("MRGg", spkrButtons)
        btnLRS = SmallButton("LRS", spkrButtons)
        btnLAS = SmallButton("LAS", spkrButtons)
        btnLAG = SmallButton("LAG", spkrButtons)
        QToolTip.add(btnVOS, "View only selected speaker's segments")
        QToolTip.add(btnVAS, "View all speakers' segments")
        QToolTip.add(btnLRS, "Listen random segments of selected speaker")
        QToolTip.add(btnLAS, "Listen all segments of selected speaker")
        QToolTip.add(btnLAG, "Listen all gaps")
        QToolTip.add(btnNSI, "Assign new speaker ID for a single segment")
        QToolTip.add(btnESIg, "Edit speaker information of all segments of selected speaker")
        QToolTip.add(btnMRG, "Merge current segment into another speaker")
        QToolTip.add(btnMRGg, "Merge all segments of selected speaker into another speaker")
        QToolTip.add(btnSRT, "Toggle speaker sorting method (chronological/aphabetical)")
        QToolTip.add(btnCLR, "Clear current speaker selection")
        self.connect(btnVOS, SIGNAL("clicked()"), self.viewBySpeaker)
        self.connect(btnVAS, SIGNAL("clicked()"), self.viewAllSpeakers)
        self.connect(btnLRS, SIGNAL("clicked()"), self.listenRandomSample)
        self.connect(btnLAS, SIGNAL("clicked()"), self.listenAll)
        self.connect(btnLAG, SIGNAL("clicked()"), self.listenGaps)
        self.connect(btnNSI, SIGNAL("clicked()"), self.assignNewSpeakerId)
        self.connect(btnESIg, SIGNAL("clicked()"), self.editSpeakerInfoGlobal)
        self.connect(btnMRG, SIGNAL("clicked()"), self.mergeSpeaker)
        self.connect(btnMRGg, SIGNAL("clicked()"), self.mergeSpeakerGlobal)
        self.connect(btnSRT, SIGNAL("clicked()"), self.spkrSelPanel.toggleSorting)
        self.connect(btnCLR, SIGNAL("clicked()"), self.spkrSelPanel.clearSelection)

        # flags for listenAll
        self._btnLAS = btnLAS
        self._stopListenAll = False
        self._listenAllThread = None
        self._btnLasColor = btnLAS.paletteBackgroundColor()

        # flags for listenGaps
        self._btnLAG = btnLAG
        self._stopListenGaps = False
        self._listenGapsThread = None
        self._btnLagColor = btnLAG.paletteBackgroundColor()
        ...
        # status bar
        self.lblToggleMarking = QLabel("normal", self.statusBar())
        self.blinker = False
        self.blinkerTimer = None
        self.blinkerColor = self.lblToggleMarking.paletteBackgroundColor()
        self.statusBar().addWidget(self.lblToggleMarking,0,True)
        ...
        menu_File_Import.insertItem("SGML: &Weblog", self.menu_File_Import_WeblogSgm)
        menu_File_Import.insertItem("SGML: &Newsgroup", self.menu_File_Import_NewsgroupSgm)
        menu_File_Export = QPopupMenu(self)
        menu_File.insertItem("&Export", menu_File_Export)
        menu_File_Export.insertItem("&CTS Style .txt", self.menu_File_Export_Txt)
        menu_File_Export.insertItem("&Transcriber", self.menu_File_Export_Transcriber)
        menu_File_Export.insertItem("SGML: &Weblog", self.menu_File_Export_WeblogSgm)
        menu_File_Export.insertItem("SGML: &Newsgroup", self.menu_File_Export_NewsgroupSgm)
        menu_File.insertSeparator()
        menu_File.insertItem("E&xit", qApp, SLOT('closeAllWindows()'))
        menu_Edit = QPopupMenu(self)
        menu_Edit.insertItem("Insert Segment", self.insertSegment)
        menu_Edit.insertItem("Delete Segment", self.deleteSegment)
        ...
        for suType in config["suTypes"]:
            f = F(suType)
            menu_Edit.insertItem("Set %s SU" % suType, f)
...
##         menu_Edit.insertItem("Set Backchannel SU", self.insertSuBackchannel)
        menu_Edit.insertItem("Unset SU", self.deleteSU)
        menu_Edit.insertSeparator()
        menu_Edit.insertItem("Insert Section Boundary", self.insertSectionBoundary)
        menu_Edit.insertItem("Delete Section Boundary", self.deleteSectionBoundaryLeft)
        menu_Edit.insertItem("Edit Section Boundary Type", self.editSectionBoundaryTypeLeft)
        menu_Edit.insertSeparator()
        menu_Edit.insertItem("&Associate transcript to audio", self.menu_Edit_Assoc)
        menu_Edit.insertItem("&Blindly associate transcript to audio", self.menu_Edit_BlindAssoc)
        ...
        self.menuId_View_ShowSpeakerPanel = menu_View.insertItem(
            "Hide speaker panel", self.menu_View_ShowSpeakerPanel)
        menu_Tools = QPopupMenu(self)
        mid = menu_Tools.insertItem("&Check spelling", self.menu_Tools_CheckSpell)
        if not capabilities["spellchecker"]:
            menu_Tools.setItemEnabled(mid, False)
        menu_Tools.insertItem("&View CTS-style transcript", self.menu_Tools_ViewCtsStyle)
        menu_Tools.insertSeparator()
        menu_Tools.insertItem("&Load configuration file", self.menu_Tools_LoadConfigFile)
        menu_Tools.insertItem("&Save configuration file", self.menu_Tools_SaveConfigFile)
        menu_Tools.insertItem("&Edit key bindings", self.menu_Tools_EditKeyBindings)
        menu_Tools.insertItem("E&dit free string key bindings", self.menu_Tools_EditFreeStringKeyBindings)
        ...
        for k,v in self.keyBindingFuncMap.items():
            aid = self.accel.insertItem(config[k])
            self.accel.connectItem(aid, v)
            self.keyBindingAccelIdMap[k] = aid
        self.fstrMap = {}            
    ...
    def _resetFreeStringKeybindings(self):
        for aid in self.fstrMap.keys():
            self.accel.removeItem(aid)
        self.fstrMap = {}
        for name in config:
            if name.startswith('FSKB_'):
                seq,s = config[name]
                aid = self.accel.insertItem(str2qkeyseq(seq))
                self.fstrMap[aid] = s
...
##                 self, "Confirm",
##                 "Please make sure that you saved your work.\n"
...
##             if res == 1: return
    ...
    def timerEvent(self, e):
        if self.wave.isSelecting():
            if self.blinker:
                self.lblToggleMarking.setPaletteForegroundColor(self.blinkerColor)
                self.blinker = False
            else:
                self.lblToggleMarking.setPaletteForegroundColor(Qt.black)
                self.blinker = True
        elif e.timerId() == self.autosaveTimer:
            self.statusBar().message("Auto-saving ...", 1000)
    ...
    def _setData(self):
        ...
        def styler(seg):
            spkr = seg['speaker']
            if ' #' not in spkr or len(spkr)==0 or spkr.split(' #')[-1][-1] != 'E':
                yield  self.tred.setColor, (Qt.lightGray,)
        self.tred.setData(self.data, lambda x:x['speaker']!=config['sectionSpeakerId'], styler=styler)
        ...
        sndfile = DummySndFile(
            ...
            channels=1,length=length,numLayers=12)
    ...
    def menu_File_New(self):
        ...
        self.setCaption("Xtrans: " + self.filename)
    ...
    def menu_File_Open(self, filename=None):
        ...
        self.statusBar().message("Opening transcript ...")

        try:
            self.data = Transcript.importTdf(filename)
        except Error, e:
            msg = "TDF import error:\n%s" % e.msg
            QMessageBox.critical(self, "Open Error", msg, "OK")
        ...
        self.statusBar().message("Transcript loaded succefully.", 2000)
    ...
    def menu_File_OpenAudio(self, filename=None):
        ...
        if os.path.isdir(filename):
            QMessageBox.critical(
                ...
                "OK")
            return
        self.statusBar().message("Opening audio file ...")
        ...
        self.statusBar().message("Audio file loaded succefully.", 2000)
    ...
    def menu_File_Import_Transcriber(self, filename=None):
        ...
        self.filenameVerified = False
        self.setCaption("Xtrans: "+os.path.basename(self.filename))
    ...
    def menu_View_ShowSpeakerPanel(self):
        if self.spkrPanel.isHidden():
            self.menu_View.changeItem(
                self.menuId_View_ShowSpeakerPanel,
                "Hide speaker panel")
            self.spkrPanel.show()
        else:
            self.menu_View.changeItem(
    ...
    def menu_Tools_ViewCtsStyle(self):
        ...
        l.setSpacing(5)
        l.setMargin(10)
        self.connect(b, SIGNAL("clicked()"), d, SLOT("accept()"))
        t.setText(codecs.getreader("utf-8")(file(filename)).read())
        d.setCaption("CTS-Style Transcript")
        d.exec_loop()
        os.unlink(filename)
    ...
    def menu_Tools_EditFreeStringKeyBindings(self):
        ...
        self._resetFreeStringKeybindings()
        #self.fstrMap = {}
        #for seq,s in d.keyBindings().items():
        #    aid = self.accel.insertItem(str2qkeyseq(seq))
        #    self.fstrMap[aid] = s
    ...
    def listenAll(self):
        ...
        if self._listenAllThread:
            ...
            self._btnLAS.setPaletteBackgroundColor(self._btnLasColor)
            return
        ...
        self._btnLAS.setPaletteBackgroundColor(Qt.red)
    ...
    def listenGaps(self):
        ...
        if self._listenGapsThread:
            ...
            self._btnLAG.setPaletteBackgroundColor(self._btnLagColor)
            return
        ...
        self._btnLAG.setPaletteBackgroundColor(Qt.red)
        ...
        if wform.getSndFile()==self.dummyAudio and \
           not config["segmentOnDummyAudio"]:
            QMessageBox.critical(
                ...
                "of a segment on the box display.",
                "OK")
            return
        if beg < 0.0:
            QMessageBox.critical(
    ...
    def toggleMarking(self):
        ...
        if self.wave.isSelecting():
            ...
            self.lblToggleMarking.setPaletteForegroundColor(Qt.black)
            self.lblToggleMarking.setText("normal")
    ...
    def handleFreeStringKeyBindings(self, aid):
        ...
        sel = self.tred.getSelection()
        if sel[0] >= 0 and sel[0] == sel[2]:
            text = s[:pos] + unicode(self.tred.selectedText()) + s[pos:]
            self.tred.insert(text)
            self.tred.setCursorPosition(sel[0], sel[1]+len(text))
        else:
            p, c = self.tred.getCursorPosition()
            self.tred.insert(s)
            self.tred.setCursorPosition(p, c+pos)
